...
#property description "Scale Invariant. With trend filter + ATR volatility filter + kill switch optional"

#include <Trade/Trade.mqh>

#resource "ml_strategy_classifier_hgb.onnx" as uchar ExtModel[]
   ...
   if(signal == SIGNAL_BUY)
     {
      ...
      ok = trade.Buy(InpLots, _Symbol, ask, sl, tp, "HGB buy");
      if(ok)
         g_bars_in_trade = 0;
     }
   else if(signal == SIGNAL_SELL)
     {
      ...
      ok = trade.Sell(InpLots, _Symbol, bid, sl, tp, "HGB sell");
      if(ok)
         g_bars_in_trade = 0;
     }
  }
