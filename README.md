# HistGradientBoosting MT5 strategy files
...
## Files
- `train_mt5_hgb_classifier.py`
- `MT5_HistGradientBoosting_Classifier_ONNX_Strategy.mq5`
...
## Running Example
```powershell
python train_mt5_hgb_classifier.py --symbol XAGUSD --timeframe M15 --bars 20000 --horizon-bars 8 --train-ratio 0.70 --output-dir output_hgb_XAGUSD_M15_h8
```

## Steps for MT5
1. Copy `ml_strategy_classifier_hgb.onnx` file near the `.mq5` file
2. Recompile the EA
3. Run tester only on the `TEST UTC` from `run_in_mt5.txt`

## Recommended EA start settings
