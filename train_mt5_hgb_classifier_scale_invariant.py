...
def split_train_test(df: pd.DataFrame, train_ratio: float) -> Tuple[pd.DataFrame, pd.DataFrame]:
    if train_ratio < 0.50 or train_ratio > 0.95:
...
def make_classifier(random_state: int = 42) -> HistGradientBoostingClassifier:
...
def class_index_map(model: HistGradientBoostingClassifier) -> Dict[int, int]:
...
def export_to_onnx(model: HistGradientBoostingClassifier, output_path: Path) -> None:
...
def write_run_in_mt5(
    ...
    txt = f"""MODEL: HistGradientBoostingClassifier (3 classes)
SYMBOL: {args.symbol}
TIMEFRAME: {args.timeframe}
...
STEPS:
1. Copy the ml_strategy_classifier_hgb.onnx file next to the .mq5 EA.
2. Recompile the EA in MetaEditor.
3. Run Strategy Tester ONLY on the TEST UTC window above.
4. For additional trend/ATR filters, start from the settings that worked best in previous tests.
"""
    (output_dir / "run_in_mt5.txt").write_text(txt, encoding="utf-8")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train a HistGradientBoosting model for MT5 and export ONNX.")
    ...
    p.add_argument("--output-dir", default="output_hgb", help="Output directory")
...
def main() -> None:
    ...
    onnx_path = output_dir / "ml_strategy_classifier_hgb.onnx"
